...
void hex_dump_stdout(const void *p, int n)
{
    ...
    for(count = 0; count < n;)
    {
        cout << uppercase << fixed << right << setw(8) << setfill('0') << hex << addr << "  "; 
        for(col = 0, addr = addr + 0x10; (count < n) && (col < 16); count++, col++)
        {
            if (col == 8) cout << ' ';
            cout << uppercase << fixed << right << setw(2) << setfill('0') << hex << (int)*(buf+count) << " "; 
            str[col] = (*(buf + count) >= 32 && *(buf + count) < 127) ? *(buf + count) : '.';
        }

        str[col] = 0;
        if (col <= 8) cout << ' ';
        cout << "  " << setfill(' ') << setw(((16 - (col))) * 3) << ""  << str << "\n"; 
    }
